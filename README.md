...
- **Port Mapping**: Port 8080 (host) → Port 80 (container)
...
- **Purpose**: MySQL 8.0 database for PHP 5.6 application
- **Container Name**: `synaptic_db_webPHP5`
- **Image**: `mysql:8.0`
- **Credentials**: Root password: `${MYSQL_ROOT_PASSWORD_PHP5}` (from .env)
- **Database**: `synaptic_db_webPHP5`
- **User**: `synaptic_db_webPHP5` / Password: `${MYSQL_PASSWORD_PHP5}` (from .env)
- **Restart Policy**: Restarts unless explicitly stopped
...
- To connect to MySQL from host, use: `docker exec -it synaptic_db_webPHP5 mysql -u synaptic_db_webPHP5 -p` (or replace PHP5 with PHP7/PHP8)
